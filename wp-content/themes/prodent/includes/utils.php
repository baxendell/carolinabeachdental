...
if (!function_exists('prodent_array_get_first')) {
	function prodent_array_get_first(&$arr, $key=true) {
...
if (!function_exists('prodent_array_get_keys_by_value')) {
	function prodent_array_get_keys_by_value($arr, $value=1, $as_string=true) {
...
if (!function_exists('prodent_array_from_list')) {
	function prodent_array_from_list($arr) {
		$new = array();
...
// $d = prodent_array_merge($a, $b);	["four", "k2"=>"six", "seven", "k1"=>"five");
if (!function_exists('prodent_array_merge')) {
	function prodent_array_merge($a1, $a2) {
...
if (!function_exists('prodent_array_insert')) {
	function prodent_array_insert_after(&$haystack, $needle, $stuff){
		...
		for ($i = 2; $i < func_num_args(); ++$i){
			...
			if (is_array($arg)) {
				if ($i==2)
					...
					$new_array = prodent_array_merge($new_array, $arg);
			} else 
				$new_array[] = $arg;
		}
		...
		$haystack = prodent_array_merge(array_slice($haystack, 0, $i, true), $new_array, array_slice($haystack, $i, null, true));
...
// @param mixed $stuff one or more arrays or scalars to be inserted into $haystack
// @return int the index at which $needle was found
if (!function_exists('prodent_array_before')) {
	function prodent_array_insert_before(&$haystack, $needle, $stuff){
		if (! is_array($haystack) ) return -1;
...
if (!function_exists('prodent_get_tag')) {
	function prodent_get_tag($text, $tag_start, $tag_end='') {
		$val = '';
...
if (!function_exists('prodent_get_tag_attrib')) {
	function prodent_get_tag_attrib($text, $tag, $attr) {
		$val = '';
...
if (!function_exists('prodent_html_decode')) {
	function prodent_html_decode($prm) {
...
if (!function_exists('prodent_get_css_position_from_values')) {
	function prodent_get_css_position_from_values($top='',$right='',$bottom='',$left='',$width='',$height='') {
		if (!is_array($top)) {
		...
		foreach ($top as $k=>$v) {
			$imp = substr($v, 0, 1);
			if ($imp == '!') $v = substr($v, 1);
			if ($v != '') $output .= ($k=='width' ? 'width' : ($k=='height' ? 'height' : 'margin-'.esc_attr($k))) . ':' . esc_attr(prodent_prepare_css_value($v)) . ($imp=='!' ? ' !important' : '') . ';';
		}
		return $output;
	}
}
...
if (!function_exists('prodent_prepare_css_value')) {
	function prodent_prepare_css_value($val) {
		if ($val != '') {
			$ed = substr($val, -1);
			if ('0'<=$ed && $ed<='9') $val .= 'px';
		}
...
if (!function_exists('prodent_parse_icons_classes')) {
	function prodent_parse_icons_classes($css) {
		$rez = array();
		if (!file_exists($css)) return $rez;
		$file = prodent_fga($css);
...
if (!function_exists('prodent_stripslashes')) {
	function prodent_stripslashes($val) {
		static $magic = 0;
		if ($magic === 0) {
...
// Get GET, POST value
if (!function_exists('prodent_get_value_gp')) {
	function prodent_get_value_gp($name, $defa='') {
		if (isset($_GET[$name]))		$rez = $_GET[$name];
		else if (isset($_POST[$name]))	$rez = $_POST[$name];
		else							$rez = $defa;
		return prodent_stripslashes($rez);
	}
}

// Get GET, POST, COOKIE value and save it (if need)
if (!function_exists('prodent_get_value_gpc')) {
	function prodent_get_value_gpc($name, $defa='', $page='', $exp=0) {
		if (isset($_GET[$name]))		 $rez = $_GET[$name];
		else if (isset($_POST[$name]))	 $rez = $_POST[$name];
...
if (!function_exists('prodent_get_value_gps')) {
	function prodent_get_value_gps($name, $defa='', $page='') {
		if (isset($_GET[$name]))		  $rez = $_GET[$name];
		else if (isset($_POST[$name]))	  $rez = $_POST[$name];
...
if (!function_exists('prodent_set_session_value')) {
	function prodent_set_session_value($name, $value, $page='') {
...
// Save value into session
if (!function_exists('prodent_del_session_value')) {
	function prodent_del_session_value($name, $page='') {
		if (!session_id()) session_start();
...
if (!function_exists('prodent_hex2rgb')) {
	function prodent_hex2rgb($hex) {
		$dec = hexdec(substr($hex, 0, 1)== '#' ? substr($hex, 1) : $hex);
		return array('r'=> $dec >> 16, 'g'=> ($dec & 0x00FF00) >> 8, 'b'=> $dec & 0x0000FF);
	}
}

if (!function_exists('prodent_hex2rgba')) {
	function prodent_hex2rgba($hex, $alpha) {
		$rgb = prodent_hex2rgb($hex);
		return 'rgba('.intval($rgb['r']).','.intval($rgb['g']).','.intval($rgb['b']).','.floatval($alpha).')';
	}
}

if (!function_exists('prodent_hex2hsb')) {
	function prodent_hex2hsb($hex, $h=0, $s=0, $b=0) {
		$hsb = prodent_rgb2hsb(prodent_hex2rgb($hex));
...
if (!function_exists('prodent_rgb2hsb')) {
	function prodent_rgb2hsb($rgb) {
		$hsb = array();
...
if (!function_exists('prodent_hsb2rgb')) {
	function prodent_hsb2rgb($hsb) {
		$rgb = array();
		$h = round($hsb['h']);
...
if (!function_exists('prodent_rgb2hex')) {
	function prodent_rgb2hex($rgb) {
		$hex = array(
			dechex($rgb['r']),
			dechex($rgb['g']),
			dechex($rgb['b'])
...
if (!function_exists('prodent_hsb2hex')) {
	function prodent_hsb2hex($hsb) {
		return prodent_rgb2hex(prodent_hsb2rgb($hsb));
	}
}
...
if (!function_exists('prodent_prepare_macros')) {
	function prodent_prepare_macros($str) {
		return str_replace(
			array("{{",  "}}",   "((",  "))",   "||"),
			array("<i>", "</i>", "<b>", "</b>", "<br>"),
			$str);
...
if (!function_exists('prodent_remove_macros')) {
	function prodent_remove_macros($str) {
		return str_replace(
			array("{{", "}}", "((", "))", "||"),
			array("",   "",   "",   "",   " "),
			$str);
...
if (!function_exists('prodent_is_on')) {
	function prodent_is_on($prm) {
		return $prm>0 || in_array(strtolower($prm), array('true', 'on', 'yes', 'show'));
	}
}
if (!function_exists('prodent_is_off')) {
	function prodent_is_off($prm) {
		return empty($prm) || $prm===0 || in_array(strtolower($prm), array('false', 'off', 'no', 'none', 'hide'));
	}
}
if (!function_exists('prodent_is_inherit')) {
	function prodent_is_inherit($prm) {
		return in_array(strtolower($prm), array('inherit'));		//array('inherit', 'default')
	}
}

// Return truncated string
if (!function_exists('prodent_strshort')) {
	function prodent_strshort($str, $maxlength, $add='...') {
...
if (!function_exists('prodent_unserialize')) {
	function prodent_unserialize($str) {
...
if (!function_exists('prodent_getimagesize')) {
	function prodent_getimagesize($url) {
		...
		$url = prodent_remove_protocol_from_url($url);
		
		foreach ($locations as $key=>$loc) {

			$loc['url'] = prodent_remove_protocol_from_url($loc['url']);
...
if (!function_exists('prodent_clear_thumb_size')) {
	function prodent_clear_thumb_size($url) {
		$pi = pathinfo($url);
...
if (!function_exists('prodent_add_thumb_size')) {
	function prodent_add_thumb_size($url, $thumb_size, $check_exists=true) {
		$pi = pathinfo($url);
		$parts = explode('-', $pi['filename']);
...
if (!function_exists('prodent_get_thumb_size')) {
	function prodent_get_thumb_size($ts) {
		static $retina = '-';
		if ($retina=='-') $retina = prodent_get_retina_multiplier() > 1 ? '-@retina' : '';
		return ($ts=='post-thumbnail' ? '' : 'prodent-thumb-') . $ts . $retina;
	}
}


// Return image url by attachment ID
if (!function_exists('prodent_get_attachment_url')) {
	function prodent_get_attachment_url($image_id, $size='full') {
		if ($image_id > 0) {
			$attach = wp_get_attachment_image_src($image_id, 'full');
			if (isset($attach[0]) && $attach[0]!='') {
				if ($size != 'full')
					$image_id = prodent_add_thumb_size($attach[0], $size);
				else
					$image_id = $attach[0];
			} else
				$image_id = '';
		} else
			$image_id = prodent_add_thumb_size($image_id, $size);
		return $image_id;
	}
}
...
if (!function_exists('prodent_get_post_image')) {
	function prodent_get_post_image($post_text='', $src=true) {
		global $post;
		$img = '';
...
if (!function_exists('prodent_get_post_audio')) {
	function prodent_get_post_audio($post_text='', $src=true) {
		global $post;
		$img = '';
		if (empty($post_text)) $post_text = $post->post_content;
		if ($src) {
			...
			$img = prodent_get_tag($post_text, '<audio', '</audio>');
			if (empty($img))
				$img = do_shortcode(prodent_get_tag($post_text, '[audio', '[/audio]'));
			if (empty($img)) {
				$img = prodent_get_tag_attrib($post_text, '[trx_widget_audio]', 'url');
				if (!empty($img))
...
if (!function_exists('prodent_get_post_video')) {
	function prodent_get_post_video($post_text='', $src=true) {
		global $post;
		$img = '';
		...
		if ($src) {
			...
			$img = prodent_get_tag($post_text, '<video', '</video>');
			if (empty($img)) {
				$sc = prodent_get_tag($post_text, '[video', '[/video]');
				if (empty($sc)) $sc = prodent_get_tag($post_text, '[trx_widget_video', '');
...
if (!function_exists('prodent_get_post_iframe')) {
	function prodent_get_post_iframe($post_text='', $src=true) {
		global $post;
		$img = '';
		if (empty($post_text)) $post_text = do_shortcode($post->post_content);
		if ($src) {
			...
			$img = prodent_get_tag($post_text, '<iframe', '</iframe>');
		return $img;
	}
}

// Add 'autoplay' feature in the video
if (!function_exists('prodent_make_video_autoplay')) {
	function prodent_make_video_autoplay($video) {
...
if (!function_exists('prodent_is_from_uploads')) {
	function prodent_is_from_uploads($url) {
		$local = false;
		$uploads_info = wp_upload_dir();
		$uploads_url = $uploads_info['baseurl'];
		$uploads_dir = $uploads_info['basedir'];
...
if (!function_exists('prodent_is_youtube_url')) {
	function prodent_is_youtube_url($url) {
		return strpos($url, 'youtu.be')!==false || strpos($url, 'youtube.com')!==false;
	}
}

// Check if URL from Vimeo
if (!function_exists('prodent_is_vimeo_url')) {
	function prodent_is_vimeo_url($url) {
...
------------------------------------------------------------------- */
if (!function_exists('prodent_init_filesystem')) {
	add_action( 'after_setup_theme', 'prodent_init_filesystem', 0);
	function prodent_init_filesystem() {
...
//if (!function_exists('prodent_fpc')) {	
//	function prodent_fpc($file, $data, $flag=0) {
...
//					throw new Exception(sprintf(esc_html__('WP Filesystem is not initialized! Put contents to the file "%s" failed', 'prodent'), $file));
//			}
//		}
//		return false;
//	}
...
//if (!function_exists('prodent_fgc')) {	
//	function prodent_fgc($file) {
//		global $wp_filesystem;
//		if (!empty($file)) {
//			if (isset($wp_filesystem) && is_object($wp_filesystem)) {
//				$file = str_replace(ABSPATH, $wp_filesystem->abspath(), $file);
...
//			} else {
//				if (prodent_is_on(prodent_get_theme_option('debug_mode')))
//					throw new Exception(sprintf(esc_html__('WP Filesystem is not initialized! Get contents from the file "%s" failed', 'prodent'), $file));
//			}
//		}
//		return '';
//	}
...
// add new content to this string and re-write it to the file if parameter $flag == FILE_APPEND!
                ...
                if (prodent_param_is_on(prodent_get_theme_option('debug_mode')))
                    throw new Exception(sprintf(esc_html__('WP Filesystem is not initialized! Put contents to the file "%s" failed', 'prodent'), $file));
            }
        }
        return false;
...
if (!function_exists('prodent_remote_get')) {
    function prodent_remote_get($file, $timeout=-1) {
...
if (!function_exists('prodent_fga')) {	
	function prodent_fga($file) {
		global $wp_filesystem;
		if (!empty($file)) {
			if (isset($wp_filesystem) && is_object($wp_filesystem)) {
				$file = str_replace(ABSPATH, $wp_filesystem->abspath(), $file);
				...
				if (prodent_is_on(prodent_get_theme_option('debug_mode')))
					throw new Exception(sprintf(esc_html__('WP Filesystem is not initialized! Get rows from the file "%s" failed', 'prodent'), $file));
			}
		}
		return array();
	}
...
if (!function_exists('prodent_esc')) {	
	function prodent_esc($file) {
...
if (!function_exists('prodent_check_min_file')) {	
	function prodent_check_min_file($file, $dir) {
		if (substr($file, -3)=='.js') {
			if (substr($file, -7)!='.min.js' && prodent_is_off(prodent_get_theme_option('debug_mode'))) {
			...
			if (substr($file, -8)!='.min.css'  && prodent_is_off(prodent_get_theme_option('debug_mode'))) {
				$dir = trailingslashit($dir);
...
if (!function_exists('prodent_get_file_dir')) {	
	function prodent_get_file_dir($file, $return_url=false) {
		...
		// Otherwise (on WordPress older then 4.7.0)
			...
			if (file_exists(($child_dir).'/'.($file)))
				$dir = ($return_url ? $child_url : $child_dir).'/'.prodent_check_min_file($file, $child_dir);
			else if (file_exists(($theme_dir).'/'.($file)))
				$dir = ($return_url ? $theme_url : $theme_dir).'/'.prodent_check_min_file($file, $theme_dir);
		}
		return apply_filters( $return_url ? 'prodent_get_file_url' : 'prodent_get_file_dir', $dir, $file );
	}
}

if (!function_exists('prodent_get_file_url')) {	
	function prodent_get_file_url($file) {
		return prodent_get_file_dir($file, true);
	}
}

// Return file extension from full name/path
if (!function_exists('prodent_get_file_ext')) {	
	function prodent_get_file_ext($file) {
		$parts = pathinfo($file);
...
if (!function_exists('prodent_get_file_name')) {	
	function prodent_get_file_name($file, $without_ext=true) {
		$parts = pathinfo($file);
...
if (!function_exists('prodent_get_folder_dir')) {	
	function prodent_get_folder_dir($folder, $return_url=false) {
...
if (!function_exists('prodent_get_folder_url')) {	
	function prodent_get_folder_url($folder) {
		return prodent_get_folder_dir($folder, true);
	}
}


// Remove protocol part from URL
if (!function_exists('prodent_remove_protocol_from_url')) {
	function prodent_remove_protocol_from_url($url) {
...
if (!function_exists('prodent_add_to_url')) {
	function prodent_add_to_url($url, $prm) {
		if (is_array($prm) && count($prm) > 0) {
