...
if (!function_exists('prodent_hovers_theme_setup3')) {
	add_action( 'after_setup_theme', 'prodent_hovers_theme_setup3', 3 );
	function prodent_hovers_theme_setup3() {

		// Add 'Buttons hover' option
		prodent_storage_set_array_after('options', 'border_radius', array(
			'button_hover' => array(
				"title" => esc_html__("Button's hover", 'prodent'),
				"desc" => wp_kses_data( __('Select hover effect to decorate all theme buttons', 'prodent') ),
				"std" => 'slide_left',
				"options" => array(
					'default'		=> esc_html__('Fade',				'prodent'),
					'slide_left'	=> esc_html__('Slide from Left',	'prodent'),
					'slide_right'	=> esc_html__('Slide from Right',	'prodent'),
					'slide_top'		=> esc_html__('Slide from Top',		'prodent'),
					'slide_bottom'	=> esc_html__('Slide from Bottom',	'prodent'),
//					'arrow'			=> esc_html__('Arrow',				'prodent'),
				),
				"type" => "select"
			),
			'image_hover' => array(
				"title" => esc_html__("Image's hover", 'prodent'),
				"desc" => wp_kses_data( __('Select hover effect to decorate all theme images', 'prodent') ),
				"std" => 'icon',
				"override" => array(
					'mode' => 'page',
				...
				"options" => array(
					'dots'	=> esc_html__('Dots',	'prodent'),
					'icon'	=> esc_html__('Icon',	'prodent'),
					'icons'	=> esc_html__('Icons',	'prodent'),
					'zoom'	=> esc_html__('Zoom',	'prodent'),
					'fade'	=> esc_html__('Fade',	'prodent'),
					'slide'	=> esc_html__('Slide',	'prodent'),
					'pull'	=> esc_html__('Pull',	'prodent'),
					'border'=> esc_html__('Border',	'prodent')
				),
				"type" => "select"
			) )
		);
...
if (!function_exists('prodent_hovers_theme_setup9')) {
	add_action( 'after_setup_theme', 'prodent_hovers_theme_setup9', 9 );
	function prodent_hovers_theme_setup9() {
		add_action( 'wp_enqueue_scripts',		'prodent_hovers_frontend_scripts', 1010 );
		add_filter( 'prodent_filter_localize_script','prodent_hovers_localize_script' );
		add_filter( 'prodent_filter_merge_scripts',	'prodent_hovers_merge_scripts' );
		add_filter( 'prodent_filter_merge_styles',	'prodent_hovers_merge_styles' );
		add_filter( 'prodent_filter_get_css', 		'prodent_hovers_get_css', 10, 4 );
	}
}
	
// Enqueue hover styles and scripts
if ( !function_exists( 'prodent_hovers_frontend_scripts' ) ) {
	//Handler of the add_action( 'wp_enqueue_scripts', 'prodent_hovers_frontend_scripts', 1010 );
	function prodent_hovers_frontend_scripts() {
		if ( prodent_is_on(prodent_get_theme_option('debug_mode')) && prodent_get_file_dir('theme-specific/theme.hovers/theme.hovers.js')!='' )
			wp_enqueue_script( 'prodent-hovers', prodent_get_file_url('theme-specific/theme.hovers/theme.hovers.js'), array('jquery'), null, true );
		if ( prodent_is_on(prodent_get_theme_option('debug_mode')) && prodent_get_file_dir('theme-specific/theme.hovers/theme.hovers.css')!='' )
			wp_enqueue_style( 'prodent-hovers',  prodent_get_file_url('theme-specific/theme.hovers/theme.hovers.css'), array(), null );
	}
}

// Merge hover effects into single js
if (!function_exists('prodent_hovers_merge_scripts')) {
	//Handler of the add_filter( 'prodent_filter_merge_scripts', 'prodent_hovers_merge_scripts' );
	function prodent_hovers_merge_scripts($list) {
...
if (!function_exists('prodent_hovers_merge_styles')) {
	//Handler of the add_filter( 'prodent_filter_merge_styles', 'prodent_hovers_merge_styles' );
	function prodent_hovers_merge_styles($list) {
...
if (!function_exists('prodent_hovers_localize_script')) {
	//Handler of the add_filter( 'prodent_filter_localize_script','prodent_hovers_localize_script' );
	function prodent_hovers_localize_script($arr) {
		$arr['button_hover'] = prodent_get_theme_option('button_hover');
		return $arr;
	}
}

// Add hover icons on the featured image
if ( !function_exists('prodent_hovers_add_icons') ) {
	function prodent_hovers_add_icons($hover, $args=array()) {
					...
					echo apply_filters( 'woocommerce_loop_add_to_cart_link',
										'<a rel="nofollow" href="' . esc_url($product->add_to_cart_url()) . '" 
														...
														class="shop_cart icon-cart-2 button add_to_cart_button'
																...
																. '">'
											. ($hover == 'shop_buttons' ? esc_html__('Buy now', 'prodent') : '')
										. '</a>',
										$product );
				}
				?><a href="<?php echo esc_url(is_object($args['cat']) ? get_term_link($args['cat']->slug, 'product_cat') : get_permalink()); ?>" aria-hidden="true" class="shop_link button icon-link"><?php
					if ($hover == 'shop_buttons') {
						if (is_object($args['cat']))
							esc_html_e('View products', 'prodent');
						else
							esc_html_e('Details', 'prodent');
					}
				?></a>
			</div>
			<?php
		...
		// Hover style 'Icon'
			...
			?><div class="icons"><a href="<?php the_permalink(); ?>" aria-hidden="true" class="icon-icon12"></a></div><?php
...
// Add styles into CSS
if ( !function_exists( 'prodent_hovers_get_css' ) ) {
	//Handler of the add_filter( 'prodent_filter_get_css', 'prodent_hovers_get_css', 10, 4 );
	function prodent_hovers_get_css($css, $colors, $fonts, $scheme='') {
