...
if (!function_exists('prodent_customizer_get_css')) {

	function prodent_customizer_get_css($colors=null, $fonts=null, $remove_spaces=true, $only_scheme='') {
		...
		if ($fonts) {
			...
			$fonts = prodent_customizer_add_theme_fonts($fonts);
...
EOT;
			$rez = apply_filters('prodent_filter_get_css', $rez, false, $fonts, '');
			$css['fonts'] = $rez['fonts'];
...
EOT;
		...
		//--------------------------------------
		if ($colors !== false) {
			$schemes = empty($only_scheme) ? array_keys(prodent_get_list_schemes()) : array($only_scheme);
	
			if (count($schemes) > 0) {
				$rez = array();
				foreach ($schemes as $scheme) {
					// Prepare colors
					if (empty($only_scheme)) $colors = prodent_get_scheme_colors($scheme);
...
.sc_layouts_title_breadcrumbs .current {
	color: {$colors['text_link']} !important;
}
...
.sc_layouts_menu_nav > li > ul:before {
    border-bottom-color: {$colors['text_hover2']} !important;
}
...
.prodent_tabs .prodent_tabs_titles li a:hover {
	color: {$colors['inverse_link']};
	background-color: {$colors['text_link']};
}
div.esg-filter-wrapper .esg-filterbutton.selected > span,
.mptt-navigation-tabs li.active a,
.prodent_tabs .prodent_tabs_titles li.ui-state-active a {
	color: {$colors['bg_color']};
	background-color: {$colors['text_dark']};
}
...
.widget_area .post_item .post_info_item a, .widget .post_item .post_info_item a {
	color: {$colors['text_link2']};
}
.widget_area .post_item .post_info_item a:hover, .widget .post_item .post_info_item a:hover {
	color: {$colors['text_link']};
}
...
EOT;
					...
					$rez = apply_filters('prodent_filter_get_css', $rez, $colors, false, $scheme);
		...
		return apply_filters( 'prodent_filter_prepare_css', $css_str, $remove_spaces );
	}
}
?>