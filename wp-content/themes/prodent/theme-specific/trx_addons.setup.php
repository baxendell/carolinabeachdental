...
------------------------------------------------------------------------------- */
if (!function_exists('prodent_trx_addons_theme_specific_setup1')) {
	add_filter( 'trx_addons_filter_components_editor', 'prodent_trx_addons_theme_specific_components');
	function prodent_trx_addons_theme_specific_components($enable=false) {
		return PRODENT_THEME_FREE
					? false		// Free version
...
if (!function_exists('prodent_trx_addons_theme_specific_setup1')) {
	add_action( 'after_setup_theme', 'prodent_trx_addons_theme_specific_setup1', 1 );
	add_action( 'trx_addons_action_save_options', 'prodent_trx_addons_theme_specific_setup1', 1 );
	function prodent_trx_addons_theme_specific_setup1() {
		if (prodent_exists_trx_addons()) {
			add_filter( 'trx_addons_cv_enable',					'prodent_trx_addons_cv_enable');
			add_filter( 'trx_addons_demo_enable',				'prodent_trx_addons_demo_enable');
			add_filter( 'trx_addons_filter_edd_themes_market',	'prodent_trx_addons_edd_themes_market_enable');
			add_filter( 'trx_addons_cpt_list',					'prodent_trx_addons_cpt_list');
			add_filter( 'trx_addons_sc_list',					'prodent_trx_addons_sc_list');
			add_filter( 'trx_addons_widgets_list',				'prodent_trx_addons_widgets_list');
		}
	}
}

// CV
if ( !function_exists( 'prodent_trx_addons_cv_enable' ) ) {
	//Handler of the add_filter( 'trx_addons_cv_enable', 'prodent_trx_addons_cv_enable');
	function prodent_trx_addons_cv_enable($enable=false) {
...
// Demo mode
if ( !function_exists( 'prodent_trx_addons_demo_enable' ) ) {
	//Handler of the add_filter( 'trx_addons_demo_enable', 'prodent_trx_addons_demo_enable');
	function prodent_trx_addons_demo_enable($enable=false) {
...
// EDD Themes market
if ( !function_exists( 'prodent_trx_addons_edd_themes_market_enable' ) ) {
	//Handler of the add_filter( 'trx_addons_filter_edd_themes_market', 'prodent_trx_addons_edd_themes_market_enable')) {
	function prodent_trx_addons_edd_themes_market_enable($enable=false) {
...
// API
if ( !function_exists( 'prodent_trx_addons_api_list' ) ) {
	//Handler of the add_filter('trx_addons_api_list',	'prodent_trx_addons_api_list');
	function prodent_trx_addons_api_list($list=array()) {
...
// CPT
if ( !function_exists( 'prodent_trx_addons_cpt_list' ) ) {
	//Handler of the add_filter('trx_addons_cpt_list',	'prodent_trx_addons_cpt_list');
	function prodent_trx_addons_cpt_list($list=array()) {
...
// Shortcodes
if ( !function_exists( 'prodent_trx_addons_sc_list' ) ) {
	//Handler of the add_filter('trx_addons_sc_list',	'prodent_trx_addons_sc_list');
	function prodent_trx_addons_sc_list($list=array()) {
...
// Widgets
if ( !function_exists( 'prodent_trx_addons_widgets_list' ) ) {
	//Handler of the add_filter('trx_addons_widgets_list',	'prodent_trx_addons_widgets_list');
	function prodent_trx_addons_widgets_list($list=array()) {
...
if ( !function_exists( 'prodent_trx_addons_menu_cache' ) ) {
	add_filter( 'trx_addons_filter_menu_cache', 'prodent_trx_addons_menu_cache');
	function prodent_trx_addons_menu_cache($list=array()) {
...
if (!function_exists('prodent_trx_addons_localize_script')) {
	...
	function prodent_trx_addons_localize_script($arr) {
		$arr['alter_link_color'] = prodent_get_scheme_color('alter_link');
		return $arr;
	}
}
...
if ( !function_exists( 'prodent_trx_addons_sc_type' ) ) {
	add_filter( 'trx_addons_sc_type', 'prodent_trx_addons_sc_type', 10, 2);
	function prodent_trx_addons_sc_type($list, $sc) {
        if ($sc == 'trx_sc_layouts_iconed_text') {
            $list[ 'accented' ] = esc_html__('Accented', 'prodent');
        }
		return $list;
	}
}
...
if ( !function_exists( 'prodent_trx_addons_sc_atts' ) ) {
	add_filter( 'trx_addons_sc_atts', 'prodent_trx_addons_sc_atts', 10, 2);
	function prodent_trx_addons_sc_atts($atts, $sc) {
...
if ( !function_exists( 'prodent_trx_addons_sc_map' ) ) {
	add_filter( 'trx_addons_sc_map', 'prodent_trx_addons_sc_map', 10, 2);
	function prodent_trx_addons_sc_map($params, $sc) {

	    if ($sc == 'trx_sc_content') {
            $params['params'][] = array(
                'param_name' => 'shadowed',
                'heading' => esc_html__('Add shadow', 'prodent'),
                'description' => wp_kses_data( __('Add shadow to this block', 'prodent') ),
                'admin_label' => true,
                "std" => "0",
                "value" => array(esc_html__('Add shadow', 'prodent') => "1" ),
                "type" => "checkbox"
            );
        }
		...
		$param = array(
			...
			'heading' => esc_html__('Color style', 'prodent'),
			'description' => wp_kses_data( __('Select color style to decorate this block', 'prodent') ),
			'edit_field_class' => 'vc_col-sm-4',
			'admin_label' => true,
			'value' => array_flip(prodent_get_list_sc_color_styles()),
			'type' => 'dropdown'
		);
...
if ( !function_exists( 'prodent_trx_addons_sow_map' ) ) {
	add_filter( 'trx_addons_sow_map', 'prodent_trx_addons_sow_map', 10, 2);
	function prodent_trx_addons_sow_map($params, $sc) {

		// Param 'color_style'
		$param = array(
			'color_style' => array(
				'label' => esc_html__('Color style', 'prodent'),
				'description' => wp_kses_data( __('Select color style to decorate this block', 'prodent') ),
				'options' => prodent_get_list_sc_color_styles(),
				'default' => 'default',
				'type' => 'select'
			)
		);
		if (in_array($sc, array('trx_sc_button')))
			prodent_array_insert_after($params, 'size', $param);
		else if (in_array($sc, array('trx_sc_action', 'trx_sc_blogger', 'trx_sc_cars', 'trx_sc_courses', 'trx_sc_content', 'trx_sc_dishes',
								'trx_sc_events', 'trx_sc_form',	'trx_sc_googlemap', 'trx_sc_portfolio', 'trx_sc_price', 'trx_sc_promo',
								'trx_sc_properties', 'trx_sc_services', 'trx_sc_team', 'trx_sc_testimonials', 'trx_sc_title',
								'trx_widget_audio', 'trx_widget_twitter')))
			prodent_array_insert_after($params, 'title_align', $param);
		return $params;
	}
}

// Add classes to the shortcode's output
if ( !function_exists( 'prodent_trx_addons_sc_output' ) ) {
	add_filter( 'trx_addons_sc_output', 'prodent_trx_addons_sc_output', 10, 4);
	function prodent_trx_addons_sc_output($output, $sc, $atts, $content) {
		...
		if (in_array($sc, array('trx_sc_action'))) {
			...
			if (!empty($atts['color_style']) && !prodent_is_inherit($atts['color_style']))
            ...
            if (!empty($atts['shadowed']) && ($atts['shadowed'] == '1'))
...
if ( !function_exists( 'prodent_trx_addons_sc_item_title_tag' ) ) {
	add_filter( 'trx_addons_filter_sc_item_title_tag', 'prodent_trx_addons_sc_item_title_tag');
	function prodent_trx_addons_sc_item_title_tag($tag='') {
		return $tag=='h1' ? 'h2' : $tag;
	}
}

// Return args for the item's button
if ( !function_exists( 'prodent_trx_addons_sc_item_button_args' ) ) {
	add_filter( 'trx_addons_filter_sc_item_button_args', 'prodent_trx_addons_sc_item_button_args', 10, 3);
	function prodent_trx_addons_sc_item_button_args($args, $sc, $sc_args) {
...
// Get clases for the actions items
if ( !function_exists( 'prodent_trx_addons_sc_price_item_link_classes' ) ) {
    add_filter( 'trx_addons_filter_sc_price_item_link_classes',	'prodent_trx_addons_sc_price_item_link_classes', 10, 2);
    function prodent_trx_addons_sc_price_item_link_classes($clases='') {
        $clases = 'sc_button sc_button_simple';
        return $clases;
    }
}

// Return theme specific title layout for the slider
if ( !function_exists( 'prodent_trx_addons_slider_title' ) ) {
	add_filter( 'trx_addons_filter_slider_title',	'prodent_trx_addons_slider_title', 10, 2 );
	function prodent_trx_addons_slider_title($title, $data) {
		$title = '';
		if (!empty($data['title'])) 
...
if ( !function_exists( 'prodent_trx_addons_sc_googlemap_styles' ) ) {
	add_filter( 'trx_addons_filter_sc_googlemap_styles',	'prodent_trx_addons_sc_googlemap_styles');
	function prodent_trx_addons_sc_googlemap_styles($list) {
		$list['dark'] = esc_html__('Dark', 'prodent');
		return $list;
	}
}
...
if ( !function_exists( 'prodent_trx_addons_tiny_mce_style_formats' ) ) {
	add_filter( 'trx_addons_filter_tiny_mce_style_formats', 'prodent_trx_addons_tiny_mce_style_formats');
	function prodent_trx_addons_tiny_mce_style_formats($style_formats) {
        ...
        if (true && is_array($style_formats) && count($style_formats)>0 ) {
            foreach ($style_formats as $k=>$v) {
                if ( $v['title'] == esc_html__('Inline', 'prodent') ) {
                    $style_formats[$k]['items'][] = array(
                        'title' => esc_html__('Promo large', 'prodent'),
                        'inline' => 'span',
                        'classes' => 'trx_addons_promo_large'
                    );
                    $style_formats[$k]['items'][] = array(
                        'title' => esc_html__('Promo small', 'prodent'),
                        'inline' => 'span',
...
if ( !function_exists( 'prodent_trx_addons_allow_override_header_image' ) ) {
	add_filter( 'prodent_filter_allow_override_header_image', 'prodent_trx_addons_allow_override_header_image' );
	function prodent_trx_addons_allow_override_header_image($allow) {
		return prodent_is_team_page() ? false : $allow;
	}
}

// Hide sidebar on the team and portfolio pages
if ( !function_exists( 'prodent_trx_addons_sidebar_present' ) ) {
	add_filter( 'prodent_filter_sidebar_present', 'prodent_trx_addons_sidebar_present' );
	function prodent_trx_addons_sidebar_present($present) {
		return !is_single() && prodent_is_team_page() ? false : $present;
	}
}

// Get thumb size for the team items
if ( !function_exists( 'prodent_trx_addons_thumb_size' ) ) {
	add_filter( 'trx_addons_filter_thumb_size',	'prodent_trx_addons_thumb_size', 10, 2);
	function prodent_trx_addons_thumb_size($thumb_size='', $type='') {
		if ($type == 'team-default')
			$thumb_size = prodent_get_thumb_size('big');
		if ($type == 'team-short')
			$thumb_size = prodent_get_thumb_size('team');
        if ($type == 'blogger-default')
            $thumb_size = prodent_get_thumb_size('blogger');
		return $thumb_size;
	}
}
...
if (!function_exists('prodent_trx_addons_meta_box_fields')) {
	add_filter( 'trx_addons_filter_meta_box_fields', 'prodent_trx_addons_meta_box_fields', 10, 2);
	function prodent_trx_addons_meta_box_fields($mb, $post_type) {
		if (defined('TRX_ADDONS_CPT_TEAM_PT') && $post_type==TRX_ADDONS_CPT_TEAM_PT) {
			$mb['email'] = array(
				"title" => esc_html__("E-mail",  'prodent'),
				"desc" => wp_kses_data( __("Team member's email", 'prodent') ),
				"std" => "",
				"details" => true,
				"type" => "text"
			);
