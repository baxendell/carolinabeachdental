...
<div class="sc_services_item<?php
	...
	echo wp_kses_post($args['featured'])=='image'
					? ' with_image' 
					: ($args['featured']=='icon' 
						? ' with_icon' 
						: ' with_number');
...
?>>
	...
	<div class="sc_services_item_info">
		<div class="sc_services_item_header">
			<h4 class="sc_services_item_title<?php if (!empty($meta['price'])) echo ' with_price'; ?>"><a href="<?php echo esc_url($link); ?>"><?php
				the_title();
				// Price
				if (!empty($meta['price'])) {
		...
		<?php if (!isset($args['hide_excerpt']) || $args['hide_excerpt']==0) { ?>
			...
			<div class="sc_services_item_button sc_item_button"><a href="<?php echo esc_url($link); ?>" class="sc_button sc_button_simple"><?php esc_html_e('More info', 'prodent'); ?></a></div>
		<?php } ?>
	</div>
</div>
<?php
