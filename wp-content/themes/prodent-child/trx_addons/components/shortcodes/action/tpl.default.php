	...
	foreach ($args['actions'] as $item) {
		...
		?>>
			...
			if (!empty($item['image'])) {
				...
				?><div class="sc_action_item_image"><img src="<?php echo esc_url($item['image']); ?>" alt="<?php echo esc_url($alt); ?>"<?php echo (!empty($attr[3]) ? ' '.trim($attr[3]) : ''); ?>></div><?php
			} else {
			...
			if (!empty($item['subtitle'])) {
				...
				?><h6 class="111 sc_action_item_subtitle"><?php
