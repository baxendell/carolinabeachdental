	...
	foreach ($args['actions'] as $item) {
		...
		?>>
			...
			if (!empty($item['image'])) {
				...
				?><div class="sc_action_item_image"><img src="<?php echo esc_url($item['image']); ?>" alt="<?php echo esc_html($alt); ?>"<?php echo (!empty($attr[3]) ? ' '.trim($attr[3]) : ''); ?>></div><?php
			} else {
				...
				if (!empty($icon)) {
					...
					?><div class="sc_action_item_icon sc_action_item_icon_type_<?php echo esc_attr($item['icon_type']); ?> <?php echo esc_attr($icon); ?>"
						...
						if (!empty($img)) {
                            ...
                            $alt = basename($img);
                            $alt = substr($alt,0,strlen($alt) - 4);
							?><img class="sc_icon_as_image" src="<?php echo esc_url($img); ?>" alt="<?php echo esc_html($alt); ?>"<?php echo (!empty($attr[3]) ? ' '.trim($attr[3]) : ''); ?>><?php
						} else {
			...
			if (!empty($item['subtitle'])) {
                ...
                ?></h6><div class="action_subtitle_separator"></div><?php
			}
			if (!empty($item['date'])) {
			...
			if (!empty($item['link']) && !empty($item['link_text'])) {
				?><a href="<?php echo esc_url($item['link']); ?>" class="<?php echo esc_attr(apply_filters('trx_addons_filter_sc_action_item_link_classes', 'sc_action_item_link sc_button')); ?>"><?php
